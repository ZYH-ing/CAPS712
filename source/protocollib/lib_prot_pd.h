...
typedef struct
{
	...
	/*! 固定档位9V判断配置---{1:判断固定档9V		0:不判断固定档9V}		*/
	volatile uint8_t b_judge_fixed_9v;
...
typedef struct
{
	/*! 作为sink 时, 屏蔽某个Fixed 档位电压请求---{true-> 屏蔽, false->else, 屏蔽档位根据prov_ignore_volx 设置 */
	volatile bool		b_prov_ignore_fixed;

	/*! 作为sink 时, 屏蔽某个Fixed 档位电压请求电压值---{50mV/单位}	*/
	volatile uint16_t	prov_ignore_volx;
	
	/*! Hard Reset的T_SRC_RECOVER 增加外部接口 */
	volatile uint16_t	t_src_recover;

	/*! PD不判断emarker线,直接发送原始PDO.true:为true时，不侦测emarker线，并发送完整pdos，false:判断 */
	volatile bool		b_pdos_ignore_emarker;
	
	/*! 电源协商的PS_RDY时间外部配置 */
	volatile uint16_t	t_pn_ps_rdy;
}pd_param_input_of_t;
...
typedef struct
{
	...
	/*! hardReset执行的端口 */
	volatile typec_instance_t hardrst_instance;
}pd_param_output_t;

typedef struct
{
	/*! 疑似PD 死了 */
	volatile bool	b_pd_derailment;
}pd_param_output_of_t;
...
{
	/*! 疑似PD 挂死计数器 */
	volatile uint8_t n_derailment_cnt;
}pd_param_private_of_t;
...
{
	/*! 扩展输入参数*/
	pd_param_input_of_t		input;

	/*! 扩展输出参数*/
	pd_param_output_of_t	output;

	/*! 扩展私有参数*/
	pd_param_private_of_t	private;
...
/**
 * @brief	<b>Description:</b> 收到升级命令，产生回调函数.
 * @details <b>Function prototype:</b> void pd_firmware_update_cb(void);
 * @param[in]  null.
 * @param[out] null.
 *
 * @return	null
...
void pd_txst_abort_process(void);
...
extern PD_BANK_OF pd_handle_of_t h_pd_of;
